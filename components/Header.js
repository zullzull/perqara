...
export default function Header({headers, loading, transparant}) {
   ...
   return (
      <header  className={`fixed z-50 w-screen ${transparant ? 'bg-white/5' : 'bg-pq-secondary'}`}>
                                       ...
                                       <div className="grid dark:text-pq-footer">
                                          {
                                             item.children.map((subitem,s)=> (
                                                <Link key={s} href={`/movies`} className={`font-semibold transition pl-2 py-2 pr-7 duration-200 ease-linear hover:bg-gray-100 hover:rounded-md ${submenu.status ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
                                                   {subitem.name}
                                                </Link>
                                             ))
                                          }
